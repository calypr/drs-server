...
func TestManagerLoadErrors(t *testing.T) {
	t.Run("missing config file", func(t *testing.T) {
		...
		if err == nil || !errors.Is(err, ErrProfileNotFound) || !strings.Contains(err.Error(), "run configure command") {
	...
	t.Run("missing profile", func(t *testing.T) {
		...
		_, err := testManager().Load("missing")
		if err == nil || !errors.Is(err, ErrProfileNotFound) || !strings.Contains(err.Error(), "need to run") {
