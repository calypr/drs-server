...
type postgresSchemaBootstrap struct {
	db *sql.DB
}
...
func (s *postgresSchemaBootstrap) validateLegacyAccessMethodScopes(ctx context.Context) error {
	hasLegacyColumns, err := s.hasLegacyAccessMethodScopeColumns(ctx)
	if err != nil {
		return err
	}
	if !hasLegacyColumns {
...
func (s *postgresSchemaBootstrap) hasLegacyAccessMethodScopeColumns(ctx context.Context) (bool, error) {
	var count int
	err := s.db.QueryRowContext(ctx, `
		SELECT COUNT(*)
