...
// RecordsServer keeps the generated test adapter's dependency surface small
// while the production server uses internalServer directly.
type RecordsServer struct{ objects *objectrecords.Service }

func NewRecordsServer(objects *objectrecords.Service) *RecordsServer {
	return &RecordsServer{objects: objects}
}
